...
class LoginEncryptionRequest(ClientBoundPacket):
    ...
    def __init__(self, public_key: bytes, verify_token: bytes):  # https://wiki.vg/Protocol#Encryption_Request
        super().__init__()
    ...
    def pack(self) -> Buffer:
        return Buffer().write_string(" " * 20).write_varint(len(self.public_key)).write_bytes(self.public_key).write_varint(len(self.verify_token)).write_bytes(self.verify_token)
...
class LoginPluginRequest(ClientBoundPacket):
    ...
    def pack(self) -> Buffer:
        return Buffer().write_varint(self.message_id).write_string(self.channel).write_bytes(self.data)
